...
def split_train_val_test():
    dir = 'Data/sourceChangeNamePathGoodAnnoAgainMyAug'
    mask_names = [i.name for i in Path(dir).glob('*.png')]
    random.shuffle(mask_names)
    val_num = int(len(mask_names) * 0.1)
    test_num = val_num

    test_mask_names = mask_names[:test_num]

    val_mask_names = mask_names[test_num:test_num + val_num]

    train_mask_names = mask_names[test_num + val_num:]

    save_dir = 'Data/aug'
    train_imgs = save_dir + '/train/images'
    train_masks = save_dir + '/train/masks'
    val_imgs = save_dir + '/val/images'
    val_masks = save_dir + '/val/masks'
    ...
    for name in train_mask_names:
        name = name[:-3]
        shutil.move(dir + '/' + name + 'jpg', train_imgs)
        shutil.move(dir + '/' + name + 'png', train_masks)

    for name in val_mask_names:
        name = name[:-3]
        shutil.move(dir + '/' + name + 'jpg', val_imgs)
        shutil.move(dir + '/' + name + 'png', val_masks)

    for name in test_mask_names:
        name = name[:-3]
        shutil.move(dir + '/' + name + 'jpg', test_imgs)
        shutil.move(dir + '/' + name + 'png', test_masks)
...
if __name__ == '__main__':
    change_name_path_in_jsons()

    check_label_in_jsons()

    labelme_jsons_to_masks_by_poly()
