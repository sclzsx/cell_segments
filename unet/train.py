...
def train(args):
    ...
    for epo in range(args.epoch):
        ...
        if args.eval:
            ...
            if miou > max_miou:
                max_miou = miou
                save_file = save_dir + '/' + args.net_name + '_best.pt'.format(epo + 1)
                torch.save(net.state_dict(), save_file)
                print('Saved checkpoint:', save_file)

        if (epo + 1) == args.epoch or (epo + 1) % args.pt_stride == 0:
...
def search_train(args):
    ...
    for i, data_dir in enumerate(data_dirs):
        args.train_set = data_dir
        args.val_set = data_dir.replace('training', 'test')
        args.test_set = data_dir.replace('training', 'test')
        args.save_suffix = 'model' + str(i + 1)
        do_train(args)
